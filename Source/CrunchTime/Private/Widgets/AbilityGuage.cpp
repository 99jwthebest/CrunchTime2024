...
void UAbilityGuage::SubscribeAbilityCommitedDelegate()
{
	...
	if (OwningASC)
	{
		if (!OwningASC->AbilityCommittedCallbacks.IsBoundToObject(this))
		{
			...
			//OwningASC->GetGameplayAttributeValueChangeDelegate(UCAttributeSet::GetManaAttribute);
		}
	}
}
...
void UAbilityGuage::ManaUpdated(const FOnAttributeChangeData& ChangeData)
{
	...
	//float Cost = UCAbilitySystemBlueprintLibrary::GetActiveAbilityManaCost(UAbilitySystemBlueprintLibrary::)
}
